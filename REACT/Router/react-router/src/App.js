// React Router DOM
// - 경로별 컨텐츠 또는 페이지를 설정할 떄 사용
// - browser Router : 컴포넌트 또는 HashRouter로 웹 브라우저 기반의 라우터
// - HashRouter : 웹 서버가 다른 URL오출 시 동일한 페이지를 보여줄 수 없는 경우에만 사용
//   주소앞에 /#/ 불음
// - Routes컴포넌트 : Route들의 모임
// - Route컴포넌트 : path(경로)와 element(컴포넌트)
// - Link컴포넌트 : to(경로), a엘리먼트는 페이지가 리로딩되므로
//   Link컴포넌트를 사용해서 리로딩을 방지
// - NavLink컴포넌트 : Link의 업그레이드 버젼, 만들어질때
//   현재 경로에 해당하는 NavLink의 Class속성이 active로 설정됨
...
function NotFound() {
  return(
    <div>
      <h2>404 Not Found! 페이지를 찾을 수 엄슴</h2>
    </div>
  );
}
