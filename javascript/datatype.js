// 데이터 타입(data Type)

// 1. 숫자
const num1 = 100;
const num2 = 0b00001;
const num3 = 0o777;
const num4 = 0xFB77;
const num5 = Infinity;
const num6 = -Infinity;
const num7 = NaN; // nan(x)
console.log(num1);
console.log(num2);
console.log(num3);
console.log(num4);
console.log(num5);
console.log(num6);
console.log(num7);

// 2. 문자열 (String)
const str1 = "hello";
const str2 = 'hello';
const str3 = `hello`; // 2015년 ES6 버전에 나온 기능
const str4 = 'hello';
const str5 = 'javascript';
const str6 = `${str4} ${str5}`;
console.log(typeof str1);
console.log(typeof str2);
console.log(typeof str3);
console.log(typeof str4);
console.log(typeof str5);
console.log(typeof str6);

// 3. 불리언 (boolean)
const bool1 = true;
const bool2 = false;
console.log(typeof bool1, typeof bool2);

// 4. undefined
// undefined는 타입이기도 하고 유일한 값이기도 하다.
var v;  // engine, 초기화 안되어서 engine이 undefined로 초기화 해줌
let l;  // engine, 초기화 안되어서 engine이 undefined로 초기화 해줌
const c = undefined;  // developer
console.log(v, typeof v);
console.log(l, typeof l);
console.log(c, typeof c);

// 5. null
// 값이 없다는 값
// null은 타입이기도 하고 유일한 값이기도 하다.
var v1 = null;
let l1 = null;
const c1 = null;
console.log(v1, typeof v1); // object
console.log(l1, typeof l1);
console.log(c1, typeof c1);

// 동적 타이밍(Dynamic Typing)
// 변수에 값이 할당될 떄 변수의 타입이 정해짐
let vv = 100; // number
console.log(vv, typeof vv);
vv = 'hello' // String
console.log(vv, typeof vv);
vv = false; // boolean
console.log(vv, typeof vv);
vv = function() {       // function
    console.log('hello');
};
console.log(vv, typeof vv);
vv = [1, 2, 3];         // object
console.log(vv, typeof vv); 
vv = {name:'홍길동', age:30}; // object
console.log(vv, typeof vv); 
vv = /abc/g;    // object
console.log(vv, typeof vv);


// ~ 심화

// instanceof 연산자 : 객체가 클래스의 인스턴스인지 여부
const vvv = new RegExp();
console.log(typeof vvv);
console.log(vvv instanceof RegExp);

// Object, Array, Function, RegExp 객체와 리터럴
// 리터럴 표기법으로 코딩을 하면 내부에서 객체생성자 표기법으로 자동변환되어 실행
const obj1 = new Object(); // 객체생성자 표기법
const obj2 = {}; // 리터럴 표기법
const arr1 = new Array(); // 객체생성자 표기법
const arr2 = []; // 리터럴 표기법
const func1 = new Function(); // 객체생성자 표기법
const func2 = function() {}; // 리터럴 표기법
const re1 = new RegExp(); // 객체생성자 표기법
const re2 = /abc/; // 리터럴 표기법

console.log(typeof new Object());
console.log(typeof {});