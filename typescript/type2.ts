...
const user4: User2 = {name: '이순신'}; // age는 optional이기 떄문에 안줘도 되나 name은 필수로 줘야함.
...
// const bird2: Bird = {name: '독수리', legCnt: 2}; // error : hasWing이 없음.
...
/* 클래스 */
// - 타입스크립트에서는 클래스의 프라퍼트들을 미리 정의해 줘야 함.
// - 생성자의 파라미터타입과 메서드의 반환타입을 정의해 줘야 함.
...
// 접근제어자ㅣ
// public : 클랙스 내외부에서 접근가능, 접근제어자 생략시 public
...
//   에러방지, 코드자동환성과 같은 타입스크립트의 장점을 살릴 수 없음.
...
// 인터페이스에 제네릭 사용(실행할 떄 들어오는 타입<T>를 사용)
interface Animal2<T> {
    name: string;
    body: T
}
...
// 제네릭에 제약 부여
// T extends string : string을 상속받은 임의의 타입
...
lengthOnly2('123'); // 길이 3
lengthOnly2(123); // 숫자 123

// keyof : 객체 프라퍼티들의 키들을 추출해 문자열 유니언 타입으로 변환 
//          = 프라퍼티명과 같은 문자열들만 받겠다!
function printKeys<T extends keyof {name: string; skill: string;}>
    (value: T): void{
    console.log(value)
}
printKeys('name');
