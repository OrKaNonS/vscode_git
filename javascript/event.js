...
//   : 이벤트가 발생하면 이벤트객체가 이벤트 핸들러(=이벤트리스터=이벤트콜백=콜백)에
...
// event capturing (이벤크 캡쳐링) : 상위 엘리먼트에서 하위 엘리먼트로 이벤트가 전달
// event prevent : 이벤트 전파 금지
// default event : 엘리먼트가 가지고 있는 기본 이벤트 ex) a엘리먼트는 누르면 이동
