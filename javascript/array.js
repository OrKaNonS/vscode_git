...
// 자바스크립트 배열의 틕성
// 1) 자바스크립트의 배열은 동적배열 (배열의 크기가 동적으로 변경된다)
// 2) 어떤 타입의 값도 배열에 넣을 수 있다.
// 3) 배열의 인덱스를 임의지정할 수 있음. (인덱스가 중간에 빠질 수 있음)
// 4) 요소의 값과 타입이 정해지지 않아도 사용할 수 있음.
// 5) 키가 인덱스값의 문자열인 객체

const arr1 = [1, 2, 3, 4, 5]; // 인덱스 0~4, ㅣlength 5
...
const arrObjleng = arrObj.lentgh;
...
// 배열 요소 연순으로
const arr20 = [1, 2, 3, 4, 5]; // [ 5, 4, 3, 2, 1 ]
...
console.log(arr21.sort()); // ascending
...
console.log(persons.sort((a, b) => a.age - b.age)); // age 순으로 ascending
console.log(persons.sort((a, b) => b.age - a.age)); // age 순으로 descending
...
console.log(persons.find(person => person.age==40)); // { name: '일길동', age: 40 }

console.log(arr21.findIndex(a => a>3)); // 0
console.log(persons.findIndex(person => person.age==40)); // 0 _ 위에 정렬을 시켜서 0이 나옴_ 모르면 찍어볼것
